...
const commands = [
  new SlashCommandBuilder()
    ...
    .addStringOption(o =>
      o.setName('game').setDescription('اسم اللعبة').setRequired(true)
    ),
  new SlashCommandBuilder()
    .setName('here')
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(o =>
      o.setName('token').setDescription('التوكن المُرسَل لروم الداشبورد').setRequired(true)
    ),
].map(cmd => cmd.toJSON());
...
client.on('guildCreate', async guild => {
  ...
  try {
    ...
    addLog(guild.id, 'DASHBOARD_CREATED', `تم إنشاء روم الداشبورد: ${dashChannel.id}`);

    await dashChannel.send(
      `👋 مرحباً **${owner.user.username}**!\n\n` +
      `🤖 البوت وصل لسيرفرك. للبدء تحتاج توكن تفعيل.\n\n` +
      ...
      `🔑 **للتفعيل:** احصل على توكن من صاحب البوت ثم استخدم:\n` +
      `\`/active TOKEN_HERE\``
    );
  } catch (err) {
...
client.on('guildDelete', guild => {
  console.log(`📤 غادر السيرفر: ${guild.name} (${guild.id})`);
...
client.on('messageDelete', async message => {
  ...
  try {
    ...
    if (server?.dashboard_channel_id) {
      warnChannel = guild.channels.cache.get(server.dashboard_channel_id);
    }
    ...
    if (count < 6) {
      await warnChannel?.send(
        ...
        `عند وصول التحذيرات لـ 6 سيتوقف البوت عن العمل في هذا السيرفر.\n` +
        `📞 تواصل مع صاحب البوت لإعادة الضبط.`
      );
    } else {
      await warnChannel?.send(
        `🚫 **تحذير 6/6 — تم إيقاف البوت**\n` +
        `وصلت للحد الأقصى من التحذيرات. البوت لن يعمل في هذا السيرفر.\n` +
...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === 'active') {
    ...
    if (tokenRow.server_id !== guildId) {
      return interaction.editReply('❌ هذا التوكن غير مخصص لهذا السيرفر.');
    }
    if (new Date(tokenRow.expires_at) <= new Date()) {
      return interaction.editReply('❌ انتهت صلاحية التوكن. اطلب توكناً جديداً من صاحب البوت.');
    }

    activateToken(tokenRow.id, guildId);
    addLog(guildId, 'TOKEN_ACTIVATED', `تم تفعيل التوكن من ${interaction.user.tag}`);

    const tierLabel = tokenRow.is_free_tier ? '🆓 مجاني (سيُرفق إعلان مع كل نشر)' : '💎 مميز';
    const expiresDate = new Date(tokenRow.expires_at).toLocaleDateString('ar-SA');

    await interaction.editReply(
      `✅ **تم تفعيل البوت بنجاح!**\n` +
      `📋 نوع التوكن: ${tierLabel}\n` +
      `⏰ ينتهي: ${expiresDate}\n\n` +
    ...
    if (server?.dashboard_channel_id) {
      ...
      await dash?.send(
        `✅ تم تفعيل البوت بواسطة **${interaction.user.tag}**\n` +
        `نوع: ${tierLabel} | ينتهي: ${expiresDate}`
      );
    }
    return;
  }

  const authorized = isServerAuthorized(guildId);
  if (!authorized) {
    return interaction.reply({
      content:
        '🔒 البوت غير مفعّل في هذا السيرفر.\nاستخدم `/active TOKEN` بعد الحصول على توكن من صاحب البوت.',
      ephemeral: true,
    });
  }

  const allowed = await canUseCommands(interaction);
  if (!allowed) {
    return interaction.reply({
      content: '🚫 ليس لديك صلاحية استخدام الأوامر في هذا السيرفر.',
      ephemeral: true,
    });
  }
  ...
  if (commandName === 'steam') {
    ...
    try {
      ...
      if (!result) {
        return interaction.editReply(`❌ ما لقيت أي حساب لـ **${gameName}** بعد 3 محاولات.`);
      }
  ...
  if (commandName === 'here') {
    ...
    try {
      ...
      await interaction.editReply(
        `✅ البوت سينشر حساب Steam مع مقطع TikTok كل **${minutes}** دقيقة في هذا الروم.` +
