...
// ─── Specific Permission Panel ────────────────────────────────────
  async function sendSpecificPermPanel(channel, guildId) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        'اختر المستخدمين أو الرولات التي يمكنها استخدام أوامر البوت.\n' +
        '> الأونر يقدر يستخدم الأوامر دائماً بغض النظر عن هذه القائمة.'
      )
      .addFields(
        { name: '👤 المستخدمون المسموح لهم', value: usersText, inline: false },
        { name: '🎭 الرولات المسموح بها', value: rolesText, inline: false }
      )
      .setFooter({ text: 'يمكنك إضافة وإزالة في أي وقت' })
      .setTimestamp();
        ...
        new ActionRowBuilder().addComponents(
          new StringSelectMenuBuilder()
            ...
            .addOptions(allowedUsers.slice(0, 25).map(u => ({
              ...
              value: u.user_id,
              description: `إزالة المستخدم ${u.user_id} من القائمة`,
            })))
        )
      );
    }

    // Remove roles row (if any)
        ...
        new ActionRowBuilder().addComponents(
          new StringSelectMenuBuilder()
            ...
            .addOptions(allowedRoles.slice(0, 25).map(r => ({
              ...
              value: r.role_id,
              description: `إزالة الرول ${r.role_id} من القائمة`,
            })))
        )
      );
    }

    // Delete existing specific-perm messages in the channel (last 5)
    try {
      const msgs = await channel.messages.fetch({ limit: 5 });
      const toDelete = msgs.filter(m => m.author.id === client.user.id && m.embeds?.[0]?.title === '👥 إدارة الصلاحيات المحددة');
...
function splitMessage(text, maxLength) {
  ...
  return chunks;
      ...
      if (id.startsWith('setup_interval_')) stopScheduler, trackedAdMessages } from './scheduler.js';
import { startDashboard } from './dashboard.js';
import {
  upsertServer, setDashboardChannel, setSetupChannel,
  setHereDefaultInterval, setCommandPermission,
